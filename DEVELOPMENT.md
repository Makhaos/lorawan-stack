...
1. `pkg/cypress/integrations` contains all test specifications.
  ...
  - One test file must have end-to-end dedicated only to a specific entity or view.
2. `pkg/cypress/{app}/shared` contains all test specification not directly related to a single entity or a view. For example, `side-navigation.spec.js` or `header.spec.js` must be placed into the `cypress/console/shared` folder because both components are present on multiple views and are partially related to the stack entities.
...
- Prefer duplicating entities with non-conflicting ids in tests instead of executing database teardown before each test. For example, when testing various scenaros for registering gateway, consider creating gateways with different ID's and EUI's instead of using a single gateway and drop the database before each test. Note: try to use this approach when possible, otherwise do not hesitate to restore database state before each test.
- Consider various stack configurations when writing end-to-end tests. Some views have different UI depending on availability of diffenent stack components. For example, the end device wizard looks different for deployments with complete cluster (NS+JS+AS) and for JS-only configuration.
...
- Smoke tests are testing complete user behavior to perform some complex and critical flow. For example, when testing registration of The Things Uno:
  1. Add test file `cypress/integration/smoke/devices/create.js`
  2. Describe the whole user story to register the device including logging in, creating an application (or using an existing one), link the application and create the end device.
- One smoke test should be encapsulated into a single `it` declaration.
- Every action must be done through the UI. We do not perform actions programmatically but try to simulate user behavior as close as possible.
